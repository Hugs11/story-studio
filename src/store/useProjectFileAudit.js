...
import { FILE_REFRESH_THROTTLE_MS, hasFreshPathSnapshot, readPathSnapshot } from './fileMetadataCache';
...
export function useProjectFileAudit(project, projectIndex = null) {
  const [statusByPath, setStatusByPath] = useState({});
  const [pending, setPending] = useState(false);
  const paths = useMemo(() => collectProjectPaths(project, projectIndex), [project, projectIndex]);
  ...
  useEffect(() => {
    ...
    async function runAudit({ allowThrottle = false } = {}) {
      ...
      const shouldShowPending = !allowThrottle || paths.some((path) => !hasFreshPathSnapshot(path, FILE_REFRESH_THROTTLE_MS));
      if (shouldShowPending) setPending(true);

      const snapshots = await Promise.all(paths.map((path) => readPathSnapshot(path, {
        maxAgeMs: allowThrottle ? FILE_REFRESH_THROTTLE_MS : 0,
      })));

      if (cancelled) return;
      const nextStatus = Object.fromEntries(snapshots.map((snapshot) => [snapshot.path, snapshot.exists]));
